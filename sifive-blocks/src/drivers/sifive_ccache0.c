...
METAL_CONSTRUCTOR(init_sifive_ccache0) {
    ...
    if (way_size != 0) {
        ways_to_disable = lim_size / way_size;

        /* If we have content in the LIM and it exceeds an integer
         * number of ways, add one to the number of ways to enable */
        if ((lim_size % way_size) != 0) {
            ways_to_disable += 1;
        }
    }
